...
import World from 'ecsy-babylon/world';
...
export default class Ecsy<
  C extends EcsyContext,
  A extends EcsyArgs<C>
> extends DomlessGlimmerComponent<C, A> {
  world: World;
  ...
  constructor(owner: unknown, args: A) {
    ...
    assert(
      'A `components` argument of type Map must be passed.',
      components instanceof Map
    );
    assert(
      'A `systems` argument of type Array must be passed.',
